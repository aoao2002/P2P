...
def process_download(sock,chunkfile, outputfile):
    ...
    if len(download_hash) > 0:
        whohas_header = struct.pack("HBBHHII", socket.htons(MAGIC), TEAM, WHOHAS, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN+len(download_hash)), socket.htonl(0), socket.htonl(0))
...
def process_inbound_udp(sock):
    ...
    if Type == WHOHAS:
        ...
        if num_concurrent_send == config.max_conn:
            # send DENIED packet
            denied_pkt = struct.pack("HBBHHII", socket.htons(MAGIC), TEAM, DENIED, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN), socket.htonl(0), socket.htonl(0))
        ...
        # send back IHAVE pkt
        if len(has_hash) > 0:
            ihave_header = struct.pack("HBBHHII", socket.htons(MAGIC), TEAM, IHAVE, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN+len(has_hash)), socket.htonl(0), socket.htonl(0))
        ...
        for has_chunkhash in has_chunkhashes:
            if has_chunkhash not in received_chunks:
                ...
                get_header = struct.pack("HBBHHII", socket.htons(MAGIC), TEAM, GET, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN+len(has_chunkhash)), socket.htonl(0), socket.htonl(0))
        ...
        # send back DATA
        data_header = struct.pack("HBBHHII", socket.htons(MAGIC), TEAM, DATA, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN), socket.htonl(1), 0)
        ...
        # send back ACK
        ack_pkt = struct.pack("HBBHHII", socket.htons(MAGIC), TEAM, ACK, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN), 0, Seq)
        ...
        if (ack_num)*MAX_PAYLOAD >= CHUNK_DATA_SIZE:
            ...
            # send next data
            data_header = struct.pack("HBBHHII", socket.htons(MAGIC),TEAM,  DATA, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN+len(next_data)), socket.htonl(ack_num+1), 0)
